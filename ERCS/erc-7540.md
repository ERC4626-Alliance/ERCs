...
- sender: sender of shares or assets before requesting
- receiver: receiver of shares or assets post claiming with normal 4626 flow
...
| **State**   | **User**                         | **Vault** |
|-------------|---------------------------------|-----------|
| Pending     | `requestDeposit(assets, owner, sender, data)` | `asset.transferFrom(owner, vault, assets)`; `pendingDepositRequest[owner] += assets` |
...
Transfers `assets` from `sender` into the Vault and submits a Request for asynchronous `deposit`. This places the Request in Pending state, with a corresponding increase in `pendingDepositRequest` for the amount `assets`. 
...
`sender` MUST equal `msg.sender` unless the `sender` has approved the `msg.sender` as an operator.
...
Assumes control of `shares` from `sender` and submits a Request for asynchronous `redeem`. This places the Request in Pending state, with a corresponding increase in `pendingRedeemRequest` for the amount `shares`. 
...
MUST support a redeem Request flow where the control of `shares` is taken from `sender` directly where `msg.sender` has ERC-20 approval over the `shares` of `sender`, or the `sender` has approved the `msg.sender` as an operator.
...
SHOULD check `msg.sender` can spend `sender` funds using allowance.

MUST revert if all of `shares` cannot be requested for `redeem` / `withdraw` (due to withdrawal limit being reached, slippage, the sender not having enough shares, etc).
...
`sender` has locked `assets` in the Vault to Request a deposit with request ID `requestId`. `owner` controls this Request, but is not necessarily the `sender`.
...
`sender` has locked `shares`, owned by `owner`, in the Vault to Request a redemption. `owner` controls this Request, but is not necessarily the `sender`.
...
The `sender` has set the `approved` status to an `operator`.
...
```solidity
    ...
    function requestDeposit(uint256 assets, address owner, address sender, bytes calldata data) external returns (uint256 requestId) {
        require(assets != 0);
        require(sender == msg.sender || isOperator[sender][msg.sender]);

        requestId = 0; // no requestId associated with this request

        asset.safeTransferFrom(sender, address(this), assets); // asset here is the Vault underlying asset

        pendingDepositRequest[owner] += assets;

        // Perform the callback
        if (data.length != 0) {
          require(ERC7540Receiver(sender).onERC7540DepositReceived(msg.sender, owner, requestId, assets, data) == ERC7540Receiver.onERC7540DepositReceived.selector, "receiver failed");
        }

        emit DepositRequest(owner, sender, requestId, msg.sender, assets);
        return requestId;
    }
