...
2. The `shareOwner` field of `redeem` and `withdraw` MUST be `msg.sender` unless the `shareOwner` has approved the `msg.sender` as an operator.
...
| **State**   | **User**                         | **Vault** |
|-------------|---------------------------------|-----------|
| Pending     | `requestDeposit(assets, requestOwner, assetOwner, data)` | `asset.transferFrom(assetOwner, vault, assets)`; `pendingDepositRequest[assetOwner] += assets` |
| Claimable   |                                 | *Internal Request fulfillment*:  `pendingDepositRequest[assetOwner] -= assets`; `claimableDepositRequest[assetOwner] += assets` |
| Claimed     | `deposit(assets, receiver)`      | `claimableDepositRequest[assetOwner] -= assets`; `vault.balanceOf[requestOwner] += shares` |
...
Multiple requests may have the same `requestId`, so a given Request is discriminated by both the `requestId` and the `requestOwner`. 
...
When `requestId==0`, the Vault MUST use purely the `requestOwner` to discriminate the request state. The Pending and Claimable state of multiple requests from the same `requestOwner` would be aggregated. If a Vault returns `0` for the `requestId` of any request, it MUST return `0` for all requests.

### Methods

#### requestDeposit

Transfers `assets` from `assetOwner` into the Vault and submits a Request for asynchronous `deposit`. This places the Request in Pending state, with a corresponding increase in `pendingDepositRequest` for the amount `assets`. 

The output `requestId` is used to partially discriminate the request along with the `requestOwner`. See [Request Ids](#request-ids) section for more info.

If the length of `data` is not 0, the Request MUST send an `onERC7540DepositReceived` callback to `requestOwner` following the interface of `ERC7540DepositReceiver` described in [Request Callbacks](#request-callbacks) section. If the length of `data` is 0, the Request MUST NOT send a callback.

When the Request is Claimable, `claimableDepositRequest` will be increased for the `requestOwner`. `deposit` or `mint` can subsequently be called by `requestOwner` to receive `shares`. A Request MAY transition straight to Claimable state but MUST NOT skip the Claimable state.
...
`requestOwner` MUST equal `msg.sender` unless the `requestOwner` has approved the `msg.sender` as an operator.
...
The amount of requested `assets` in Pending state for the `requestOwner` to `deposit` or `mint`.
...
The amount of requested `assets` in Claimable state for the `requestOwner` to `deposit` or `mint`.
...
Assumes control of `shares` from `shareOwner` and submits a Request for asynchronous `redeem`. This places the Request in Pending state, with a corresponding increase in `pendingRedeemRequest` for the amount `shares`. 
...
MUST support a redeem Request flow where the control of `shares` is taken from `shareOwner` directly where `msg.sender` has ERC-20 approval over the `shares` of `shareOwner`, or the `shareOwner` has approved the `msg.sender` as an operator.

If the length of `data` is not 0, the Request MUST send an `onERC7540RedeemReceived` callback to `requestOwner` following the interface of `ERC7540RedeemReceiver` described in [Request Callbacks](#request-callbacks) section. If the length of `data` is 0, the Request MUST NOT send a callback.

When the Request is Claimable, `claimableRedeemRequest` will be increased for the `requestOwner`. `redeem` or `withdraw` can subsequently be called by `requestOwner` to receive `assets`. A Request MAY transition straight to Claimable state but MUST NOT skip the Claimable state.
...
SHOULD check `msg.sender` can spend `shareOwner` funds using allowance.
...
The amount of requested `shares` in Pending state for the `requestOwner` to `redeem` or `withdraw`.
...
The amount of requested `shares` in Claimable state for the `requestOwner` to `redeem` or `withdraw`.
...
Returns `true` if the `operator` is approved as an operator for an `owner`, either `assetOwner` or `shareOwner`. 
...
Implementations MUST support an additional overloaded `deposit` and `mint` method on the specification from [ERC-4626](./eip-4626.md), with an additional `requestOwner` input of type `address`:

- `deposit(uint256 assets, address receiver, address requestOwner)`
- `mint(uint256 shares, address receiver, address requestOwner)`

The `requestOwner` field is used to look up the Request for which the `assets` should be claimed.

Calls MUST revert unless `msg.sender` is either equal to `requestOwner` or an operator approved by `requestOwner`.

When the `Deposit` event is emitted, the first parameter MUST be the `sender`, and the second parameter MUST be the `requestOwner`.

### Events

#### DepositRequest

`assetOwner` has locked `assets` in the Vault to Request a deposit with request ID `requestId`. `requestOwner` controls this Request. `sender` is the caller of the `requestDeposit` which may not be equal to the `assetOwner`.
...
`sender` has locked `shares`, owned by `shareOwner`, in the Vault to Request a redemption. `requestOwner` controls this Request, but is not necessarily the `shareOwner`.
...
`sender` is the `msg.sender` of the original `requestDeposit` call. `requestOwner` is the `owner` of the `requestDeposit`. `requestId` is the output `requestId` of the `requestDeposit`, `assets` is the amount transferred on `requestDeposit`, and `data` is the `data` of the `requestDeposit`.
...
`sender` is the `msg.sender` of the original `requestRedeem` call. `requestOwner` is the `owner` of the `requestRedeem`. `requestId` is the output `requestId` of the `requestRedeem`, `shares` is the amount transferred on `requestRedeem`, and `data` is the `data` of the `requestRedeem`.
...
```solidity
    ...
    function requestDeposit(uint256 assets, address requestOwner, address assetOwner, bytes calldata data) external returns (uint256 requestId) {
        require(assets != 0);
        require(assetOwner == msg.sender || isOperator[assetOwner][msg.sender]);

        requestId = 0; // no requestId associated with this request

        asset.safeTransferFrom(assetOwner, address(this), assets); // asset here is the Vault underlying asset

        pendingDepositRequest[assetOwner] += assets;

        // Perform the callback
        if (data.length != 0) {
          require(ERC7540Receiver(receiver).onERC7540DepositReceived(msg.sender, requestOwner, requestId, assets, data) == ERC7540Receiver.onERC7540DepositReceived.selector, "receiver failed");
        }

        emit DepositRequest(requestOwner, assetOwner, requestId, msg.sender, assets);
        return requestId;
    }
    ...
    function deposit(uint256 assets, address receiver, address requestOwner) external returns (uint256 shares) {
        require(assets != 0);
        require(requestOwner == msg.sender || isOperator[requestOwner][msg.sender]);

        claimableDepositRequest[requestOwner] -= assets; // underflow would revert if not enough claimable assets
        ...
        emit Deposit(requestOwner, receiver, assets, shares);
    }
