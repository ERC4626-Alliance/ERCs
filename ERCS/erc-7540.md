...
- owner: owner of the Request, who can manage any actions related to the Request including claiming the `assets` or `shares`
- receiver: future receiver of claimable shares or assets
...
| **State**   | **User**                         | **Vault** |
|-------------|---------------------------------|-----------|
| Pending     | `requestDeposit(assets, owner, receiver, data)` | `asset.transferFrom(owner, vault, assets)`; `pendingDepositRequest[owner] += assets` |
...
Transfers `assets` from `receiver` into the Vault and submits a Request for asynchronous `deposit`. This places the Request in Pending state, with a corresponding increase in `pendingDepositRequest` for the amount `assets`. 

The output `requestId` is used to partially discriminate the request along with the `owner`. See [Request Ids](#request-ids) section for more info.

If the length of `data` is not 0, the Request MUST send an `onERC7540DepositReceived` callback to `owner` following the interface of `ERC7540DepositReceiver` described in [Request Callbacks](#request-callbacks) section. If the length of `data` is 0, the Request MUST NOT send a callback.

When the Request is Claimable, `claimableDepositRequest` will be increased for the `owner`. `deposit` or `mint` can subsequently be called by `owner` to receive `shares`. A Request MAY transition straight to Claimable state but MUST NOT skip the Claimable state.
...
`receiver` MUST equal `msg.sender` unless the `receiver` has approved the `msg.sender` as an operator.
...
Assumes control of `shares` from `receiver` and submits a Request for asynchronous `redeem`. This places the Request in Pending state, with a corresponding increase in `pendingRedeemRequest` for the amount `shares`. 
...
MUST support a redeem Request flow where the control of `shares` is taken from `receiver` directly where `msg.sender` has ERC-20 approval over the `shares` of `receiver`, or the `receiver` has approved the `msg.sender` as an operator.

If the length of `data` is not 0, the Request MUST send an `onERC7540RedeemReceived` callback to `owner` following the interface of `ERC7540RedeemReceiver` described in [Request Callbacks](#request-callbacks) section. If the length of `data` is 0, the Request MUST NOT send a callback.

When the Request is Claimable, `claimableRedeemRequest` will be increased for the `owner`. `redeem` or `withdraw` can subsequently be called by `owner` to receive `assets`. A Request MAY transition straight to Claimable state but MUST NOT skip the Claimable state.
...
SHOULD check `msg.sender` can spend `receiver` funds using allowance.

MUST revert if all of `shares` cannot be requested for `redeem` / `withdraw` (due to withdrawal limit being reached, slippage, the receiver not having enough shares, etc).
...
`receiver` has locked `assets` in the Vault to Request a deposit with request ID `requestId`. `owner` controls this Request. `sender` is the caller of the `requestDeposit` which may not be equal to the `receiver`.
...
`receiver` has locked `shares`, owned by `owner`, in the Vault to Request a redemption. `owner` controls this Request, but is not necessarily the `receiver`.
...
The `receiver` has set the `approved` status to an `operator`.
...
All methods that initiate a request (including `requestId==0`) include a `data` parameter, which if nonzero length MUST send a callback to the owner. 
...
`sender` is the `msg.sender` of the original `requestDeposit` call. `receiver` is the future receiver of shares of the `requestDeposit`. `requestId` is the output `requestId` of the `requestDeposit`, `assets` is the amount transferred on `requestDeposit`, and `data` is the `data` of the `requestDeposit`.
...
`sender` is the `msg.sender` of the original `requestRedeem` call. `receiver` is the future receiver of shares of the `requestRedeem`. `requestId` is the output `requestId` of the `requestRedeem`, `shares` is the amount transferred on `requestRedeem`, and `data` is the `data` of the `requestRedeem`.
...
```solidity
    ...
    function requestDeposit(uint256 assets, address owner, address receiver, bytes calldata data) external returns (uint256 requestId) {
        require(assets != 0);
        require(receiver == msg.sender || isOperator[receiver][msg.sender]);

        requestId = 0; // no requestId associated with this request

        asset.safeTransferFrom(receiver, address(this), assets); // asset here is the Vault underlying asset

        pendingDepositRequest[owner] += assets;

        // Perform the callback
        if (data.length != 0) {
          require(ERC7540Receiver(receiver).onERC7540DepositReceived(msg.sender, receiver, requestId, assets, data) == ERC7540Receiver.onERC7540DepositReceived.selector, "receiver failed");
        }

        emit DepositRequest(owner, receiver, requestId, msg.sender, assets);
        return requestId;
    }
