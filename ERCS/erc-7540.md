---
eip: 7540
title: 2-Step Asynchronous ERC-4626 Tokenized Vaults
description: Extension of ERC-4626 with 2-step asynchronous deposit and redemption support
...
The following standard extends [ERC-4626](./eip-4626.md) by adding support for 2-step asynchronous deposit and redemption flows. The async flows are called Requests.

New methods are added to asynchronously Request a deposit or redemption, and view the pending status of the Request. The existing `deposit`, `mint`, `withdraw`, and `redeem` ERC-4626 methods are used for executing the second step for Claimable Requests. 
...
- Pending: the state where the first step of a Request has been made but is not yet Claimable
...
- Claimed: the state where the second step of a Request is finalized by the user and the user receives the output token (e.g. `shares` for a deposit Request)
...
The 2-step approach used in the standard may be abstracted into a 1-step approach from the user perspective through the use of routers, relayers, message signing, or account abstraction. A bespoke 1-step standard could also be developed which uses a push instead of pull flow for tokens.
